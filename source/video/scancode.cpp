...
// SDL2 to CEGUI scancode lookup table
static int g_lookup[SDL_NUM_SCANCODES] = {
...
std::vector<Uint32> g_convert_sdl_textinput_to_cegui(std::string utf8str)
{
    ...
    char* outbuf = reinterpret_cast<char*>(utf32buf);
    size_t n = SDL_iconv(cd, &inbuf, &inbytesleft, &outbuf, &outbytesleft);
    if (n == size_t(-1)) {
